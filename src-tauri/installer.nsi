...
Section Install
  ...
  {{#each file_associations as |association| ~}}
    {{#each association.ext as |ext| ~}}
       !insertmacro APP_ASSOCIATE "{{ext}}" "{{or association.name ext}}" "{{association-description association.description ext}}" "$INSTDIR\icons\capsage-document.ico,0" "Open with ${PRODUCTNAME}" "$INSTDIR\${MAINBINARYNAME}.exe $\"%1$\""
    {{/each}}
  {{/each}}
  ...
  ; Copy icon.ico to a world-readable non-profile location so that
  ; shortcut IconLocation paths don't get stored as %USERPROFILE%\...
  ; (Windows Explorer does NOT env-expand in the IconLocation field.)
  CreateDirectory "C:\Users\Public\${PRODUCTNAME}"
...
Section Uninstall
  ...
  ${If} $UpdateMode <> 1
    ...
    ; Remove the public-location icon file we stashed during install
    Delete "C:\Users\Public\${PRODUCTNAME}\icon.ico"
    RmDir  "C:\Users\Public\${PRODUCTNAME}"
  ${EndIf}
...
Function CreateOrUpdateStartMenuShortcut
  ...
  !if "${STARTMENUFOLDER}" != ""
    ...
    CreateShortcut "$SMPROGRAMS\$AppStartMenuFolder\${PRODUCTNAME}.lnk" "$INSTDIR\${MAINBINARYNAME}.exe" "" "C:\Users\Public\${PRODUCTNAME}\icon.ico" 0
    !insertmacro SetLnkAppUserModelId "$SMPROGRAMS\$AppStartMenuFolder\${PRODUCTNAME}.lnk"
  !else
    CreateShortcut "$SMPROGRAMS\${PRODUCTNAME}.lnk" "$INSTDIR\${MAINBINARYNAME}.exe" "" "C:\Users\Public\${PRODUCTNAME}\icon.ico" 0
